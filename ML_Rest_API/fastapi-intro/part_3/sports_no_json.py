...
def register_fan(fan: Fan):
    fans.append(fan)
    response = {"Message": "Fan registered successfully"}
    return response
...
def register_game(game: Game):
    games.append(game)
    response = {"Message": "Game scheduled successfully"}
    return response
...
def get_games_by_date(
    ...
    response = {"Message": filtered_games}
    return response
...
def get_team_next_game(team_name: str = Path(..., description="Name of the team")):
    ...
    response = {"Next Game": next_game}
    return response


if __name__ == "__main__":
    import uvicorn
    uvicorn.run("sports_no_json:app", host="0.0.0.0", port=8000, reload=True)